    ...
    if (renderConfig?.crackUseNoise) {
        ...
        for (let y = 0; y < canvasH; y++) {
            for (let x = 0; x < canvasW; x++) {
                ...
                if (noiseMaskData) {
                    const maskAlpha = noiseMaskData[baseIndex] > 0 ? 1 : 0;
                    const finalAlpha = maskAlpha ? alpha : 0;
                    if (finalAlpha <= 0) {
                        data[idx + 3] = 0;
                        continue;
                    }
        ...
        if (!hasCoverage) {
            ...
            for (let y = 0; y < canvasH; y++) {
                for (let x = 0; x < canvasW; x++) {
                    ...
                    const maskAlpha = noiseMaskData ? (noiseMaskData[baseIndex] > 0 ? 1 : 0) : 1;
                    const finalAlpha = maskAlpha ? srcAlpha : 0;
                    if (finalAlpha <= 0) {
                        data[idx] = 0;
                        data[idx + 1] = 0;
                        data[idx + 2] = 0;
