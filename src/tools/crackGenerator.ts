...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const density = (typeof crackCfg.divisionDensity === 'number' && isFinite(crackCfg.divisionDensity) && crackCfg.divisionDensity > 0)
        ? crackCfg.divisionDensity
        : (600 / (512 * 512));
    const autoDivisions = Math.max(16, Math.round(area * density));
    const divisions = (typeof crackCfg.divisions === 'number' && crackCfg.divisions > 0) ? crackCfg.divisions : autoDivisions;
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        const localSampleBucketId = (screenX: number, screenY: number) => {
        ...
        sampleBucketId = localSampleBucketId;

        let maskBucketCoverage: number[] | null = null;
        if (debugMaskData) {
            maskBucketCoverage = new Array<number>(buckets).fill(0);
            for (let y = 0; y < height; y++) {
                for (let x = 0; x < width; x++) {
                    const baseIndex = y * width + x;
                    if (debugMaskData[baseIndex] === 0) continue;
                    const bucketId = localSampleBucketId(x + 0.5, y + 0.5);
                    maskBucketCoverage[bucketId]++;
                }
            }
        }
        ...
        if (maskBucketCoverage) {
            let hasCoverage = false;
            for (const bucket of activeBuckets) {
                if (maskBucketCoverage[bucket] > 0) {
                    hasCoverage = true;
                    break;
                }
            }
            if (!hasCoverage) {
                let bestBucket = -1;
                let bestCount = 0;
                for (let i = 0; i < maskBucketCoverage.length; i++) {
                    const count = maskBucketCoverage[i];
                    if (count > bestCount) {
                        bestCount = count;
                        bestBucket = i;
                    }
