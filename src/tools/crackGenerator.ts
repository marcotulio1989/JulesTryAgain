...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        try {
            if (renderConfig) {
                const detectedSummary: Record<number, number> = {};
                for (let i = 0; i < countsAll.length; i++) {
                    detectedSummary[i] = countsAll[i];
                }
                (renderConfig as any).detectedNoiseBuckets = detectedSummary;
            }
        } catch (err) {
            // ignore serialization issues when updating config snapshot
        }
        ...
        const shouldExposeMask = !!(renderConfig && renderConfig.showNoiseDelimitations);
        noiseMaskData = shouldExposeMask ? new Uint8Array(canvasW * canvasH) : null;
        noiseMaskHits = 0;
        ...
        const sampleMaskWeight = (screenX: number, screenY: number): number => {
            if (!sampleBucketId) return 1;
            const bucketId = sampleBucketId(screenX, screenY);
            if (!activeBuckets || !activeBuckets.has(bucketId)) return 0;
            ...
            if (dist > crackBandWidth) return 0;
            ...
            return modulation > 0.02 ? modulation : 0;
        };
        ...
        for (let y = 0; y < canvasH; y++) {
            for (let x = 0; x < canvasW; x++) {
                ...
                const modulation = sampleMaskWeight(screenX, screenY);
                if (modulation <= 0) {
                    data[idx + 3] = 0;
                    continue;
                }
                const finalAlpha = Math.max(0, Math.min(255, Math.round(alpha * modulation)));
                if (finalAlpha <= 0) {
                    data[idx + 3] = 0;
                    continue;
                }
                ...
                if (noiseMaskData) {
                    const maskIndex = y * canvasW + x;
                    const sampleAlpha = Math.max(0, Math.min(255, Math.round(modulation * 255)));
                    if (sampleAlpha > noiseMaskData[maskIndex]) {
                        if (noiseMaskData[maskIndex] === 0 && sampleAlpha > 0) noiseMaskHits++;
                        noiseMaskData[maskIndex] = sampleAlpha;
                    }
                }
        ...
        if (!hasCoverage) {
            ...
            for (let y = 0; y < canvasH; y++) {
                for (let x = 0; x < canvasW; x++) {
                    ...
                    const finalAlpha = Math.max(0, Math.min(255, Math.round(srcAlpha * modulation)));
                    if (finalAlpha <= 0) {
                        data[idx] = 0;
                        data[idx + 1] = 0;
                        data[idx + 2] = 0;
