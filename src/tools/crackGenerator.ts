...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const patternScaleRaw = (typeof crackCfg.patternScale === 'number' && isFinite(crackCfg.patternScale))
        ? crackCfg.patternScale
        : 1;
    const patternScale = Math.max(0.25, Math.min(4, patternScaleRaw || 1));
    const scaledDivisions = Math.max(8, Math.round(divisions * patternScale * patternScale));
    const scaledThickness = Math.max(0.25, thickness / patternScale);
    const scaledDilate = Math.max(0, dilateRadius / patternScale);

    const data = generateVoronoiCrackImage(canvasW, canvasH, {
        divisions: scaledDivisions,
        thickness: scaledThickness,
        dilateRadius: scaledDilate,
        seed,
        scale: quality,
        color: [24, 24, 24],
    });
