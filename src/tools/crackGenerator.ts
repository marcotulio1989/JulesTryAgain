...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    if (canvasW > maxCanvasDimension || canvasH > maxCanvasDimension || canvasW * canvasH > maxCanvasPixels) {
        const limitForWidth = maxCanvasDimension / Math.max(1, width);
        const limitForHeight = maxCanvasDimension / Math.max(1, height);
        const limitForPixels = Math.sqrt(maxCanvasPixels / Math.max(1, width * height));
        const limitQualityRaw = Math.min(limitForWidth, limitForHeight, limitForPixels);
        if (isFinite(limitQualityRaw) && limitQualityRaw > 0) {
            const previousQuality = quality;
            const forcedQuality = Math.max(1e-6, Math.min(quality, limitQualityRaw));
            if (forcedQuality < quality) {
                quality = forcedQuality;
                if (typeof console !== 'undefined' && console.warn && forcedQuality < minQuality - 1e-3) {
                    console.warn('[crackGenerator] Crack raster quality forced below configured minimum', {
                        requested: previousQuality,
                        applied: quality,
                        width,
                        height,
                        maxCanvasDimension,
            ...
            canvasW = Math.max(1, Math.floor(width * quality));
            canvasH = Math.max(1, Math.floor(height * quality));
        }
        const enforceHardLimits = () => {
            let adjusted = false;
            if (canvasW > maxCanvasDimension) {
                const factor = Math.max(1, canvasW / maxCanvasDimension);
                quality = Math.max(1e-6, quality / factor);
                canvasW = Math.max(1, Math.floor(width * quality));
                canvasH = Math.max(1, Math.floor(height * quality));
                adjusted = true;
            }
            if (canvasH > maxCanvasDimension) {
                const factor = Math.max(1, canvasH / maxCanvasDimension);
                quality = Math.max(1e-6, quality / factor);
                canvasW = Math.max(1, Math.floor(width * quality));
                canvasH = Math.max(1, Math.floor(height * quality));
                adjusted = true;
            ...
            if (pixels > maxCanvasPixels) {
                const factor = Math.sqrt(pixels / maxCanvasPixels);
                if (factor > 1) {
                    quality = Math.max(1e-6, quality / factor);
                    canvasW = Math.max(1, Math.floor(width * quality));
                    canvasH = Math.max(1, Math.floor(height * quality));
        ...
        let guard = 0;
        while (guard < 4 && (canvasW > maxCanvasDimension || canvasH > maxCanvasDimension || canvasW * canvasH > maxCanvasPixels)) {
            if (!enforceHardLimits()) break;
            guard++;
        }
        if (canvasW > maxCanvasDimension) {
            canvasW = maxCanvasDimension;
            quality = canvasW / Math.max(1, width);
            canvasH = Math.max(1, Math.floor(height * quality));
        }
        if (canvasH > maxCanvasDimension) {
            canvasH = maxCanvasDimension;
            quality = Math.min(quality, canvasH / Math.max(1, height));
            canvasW = Math.max(1, Math.floor(width * quality));
        }
        if (canvasW * canvasH > maxCanvasPixels) {
            const factor = Math.sqrt((canvasW * canvasH) / maxCanvasPixels);
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        const activeBucketList = Array.from(activeBuckets || []);
        const bucketSlot = new Map<number, number>();
        activeBucketList.forEach((bucketId, index) => bucketSlot.set(bucketId, index));
        ...
        const coarseBase = activeBucketList.map(() => new Float32Array(coarseW * coarseH));
        const coarseFine = activeBucketList.map(() => new Float32Array(coarseW * coarseH));
        ...
        for (let gy = 0; gy < coarseH; gy++) {
            ...
            for (let gx = 0; gx < coarseW; gx++) {
                ...
                for (let bi = 0; bi < activeBucketList.length; bi++) {
                    const bucketId = activeBucketList[bi];
