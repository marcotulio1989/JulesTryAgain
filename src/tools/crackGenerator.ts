...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    let bucketIdMap: Uint8Array | null = null;
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        bucketIdMap = new Uint8Array(width * height);
        ...
        for (let y = 0; y < height; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                bucketIdMap![baseIndex] = bucketId;
        ...
        for (let y = 0; y < canvasH; y++) {
            for (let x = 0; x < canvasW; x++) {
                ...
                const bucketId = bucketIdMap ? bucketIdMap[baseIndex] : 0;
