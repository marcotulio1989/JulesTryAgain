...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const wantNoiseMask = !!(renderConfig?.crackUseNoise && renderConfig?.showNoiseDelimitations);
    const noiseMaskData = wantNoiseMask ? new Uint8Array(width * height) : null;
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        for (let y = 0; y < canvasH; y++) {
            for (let x = 0; x < canvasW; x++) {
                ...
                const baseIndex = baseY * width + baseX;
                let bucketId = sampleBucketId!(screenX, screenY);
                let allowPixel = true;
                if (debugMaskData && debugMaskData[baseIndex] === 0) {
                    allowPixel = false;
                }
                if (allowPixel && !activeBuckets!.has(bucketId)) {
                    allowPixel = false;
                }
                if (!allowPixel) {
                    data[idx + 3] = 0;
