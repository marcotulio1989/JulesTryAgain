...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const seed = Math.floor((renderConfig?.crackSeed ?? Date.now())) >>> 0;
    const baseDivisions = (typeof crackCfg.divisions === 'number' && crackCfg.divisions > 0) ? crackCfg.divisions : 400;
    const patternScaleRaw = (typeof crackCfg.patternScale === 'number' && isFinite(crackCfg.patternScale)) ? crackCfg.patternScale : 1;
    const patternScale = Math.max(0.25, Math.min(4, patternScaleRaw || 1));
    const divisions = Math.max(8, Math.min(5000, Math.round(baseDivisions * patternScale)));
    const thicknessRaw = (typeof crackCfg.thickness === 'number' && crackCfg.thickness > 0) ? crackCfg.thickness : 6;
    const thickness = Math.max(0.5, Math.min(40, thicknessRaw));
    const dilateRaw = (typeof crackCfg.dilateRadius === 'number' && crackCfg.dilateRadius >= 0) ? crackCfg.dilateRadius : 2;
    const dilateRadius = Math.max(0, Math.min(40, dilateRaw));
