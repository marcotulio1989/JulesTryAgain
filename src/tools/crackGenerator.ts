...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    if (renderConfig?.showNoiseDelimitations && renderConfig?.crackUseNoise) {
            try {
                ...
                out.debugRegion = {
                    ...
                    minY,
                    quality,
                    activeBuckets: Array.from({ length: buckets }, (_, i) => i),
                };
                return out;
            } catch (e) {
    ...
    let debug_buckets = 0;
    let debug_activeBuckets: number[] | null = null;
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        debug_activeBuckets = Array.from(activeBuckets).sort((a, b) => a - b);
    ...
    if (attachDebugRegionRequested && debug_regionMap) {
        out.debugRegion = {
            ...
            minY,
            quality,
            activeBuckets: debug_activeBuckets || undefined,
        };
    }
    if (crashMaskData) {
        out.crashMask = {
