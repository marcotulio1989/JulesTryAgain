...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const rawPatternScale = (typeof (crackCfg as any).patternScale === 'number' && isFinite((crackCfg as any).patternScale)) ? (crackCfg as any).patternScale : 1;
    const patternScale = Math.max(0.25, Math.min(4, rawPatternScale || 1));
    const divisions = Math.max(8, Math.round(baseDivisions * patternScale));
    ...
    const rawScaleMultiplier = (typeof (crackCfg as any).scale === 'number' && isFinite((crackCfg as any).scale)) ? (crackCfg as any).scale : 1;
    const scaleMultiplier = Math.max(0.25, Math.min(4, rawScaleMultiplier || 1));
