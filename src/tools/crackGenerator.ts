...
export function generateVoronoiCrackImage(width: number, height: number, options: CrackGeneratorOptions): Uint8ClampedArray {
    ...
    const divisions = Math.max(8, Math.min(12000, Math.round(options.divisions)));
...
export function generateCrackRaster(options: CrackRasterOptions): CrackRaster | null {
    ...
    const areaFactor = Math.max(1, (width * height) / baseArea);
    const qualityFactor = Math.max(1, quality * quality);
    const scaledDivisionsRaw = divisions * areaFactor * qualityFactor;
    const scaledDivisions = Math.max(64, Math.min(12000, Math.round(scaledDivisionsRaw)));
    ...
    if (renderConfig?.crackUseNoise) {
        ...
        const maskedCounts = new Array<number>(buckets).fill(0);
        let maskedSamples = 0;
        for (let ry = 0; ry < debug_regionH; ry++) {
            for (let rx = 0; rx < debug_regionW; rx++) {
                const sampleX = ((rx + 0.5) / debug_regionW) * width;
                ...
                counts[id]++;
                if (debugMaskData) {
                    const maskX = Math.max(0, Math.min(width - 1, Math.floor(sampleX)));
                    const maskY = Math.max(0, Math.min(height - 1, Math.floor(sampleY)));
                    const maskIndex = maskY * width + maskX;
                    if (debugMaskData[maskIndex] > 0) {
                        maskedCounts[id]++;
                        maskedSamples++;
                    }
                }
        ...
        const useMaskedCounts = !!(debugMaskData && maskedSamples > 0);
        const stats = counts.map((total, i) => ({
            i,
            total,
            masked: maskedCounts[i],
            metric: useMaskedCounts ? maskedCounts[i] : total,
        }));
        let picked: { i: number; total: number; masked: number; metric: number }[] = [];
        if (strategy === 'largest') {
            picked = stats.slice().sort((a, b) => (b.metric - a.metric) || (b.total - a.total)).slice(0, maxActive);
            ...
            const pool = stats.filter(s => !useMaskedCounts || s.metric > 0);
            const arr = (pool.length > 0 ? pool : stats).slice();
        ...
        if (useMaskedCounts && activeBuckets.size > 0) {
            ...
            if (!intersects) {
                ...
                for (let b = 0; b < buckets; b++) {
                    if (maskedCounts[b] > fallbackValue) {
                        fallbackValue = maskedCounts[b];
                        fallbackId = b;
                    }
                }
                if (fallbackId >= 0) {
                    activeBuckets = new Set<number>([fallbackId]);
                }
            }
        }
        ...
        // After bucket filtering, ensure we didn't zero-out entire image
        let any = false;
        ...
        } else if (crashMaskData && crashMaskHits === 0 && debugMaskData) {
            for (let i = 0; i < crashMaskData.length; i++) {
                crashMaskData[i] = debugMaskData[i] > 0 ? 255 : 0;
            }
        }
