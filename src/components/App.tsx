...
const App: React.FC = () => {
    ...
    const coverageNormalized = Math.min(1, Math.max(0, crackAreaCoverage));
    const coverageEased = Math.pow(coverageNormalized, 0.75);
    const activeBucketsForDisplay = Math.max(1, Math.min(bucketsForDisplay, Math.round(1 + coverageEased * (bucketsForDisplay - 1))));
    ...
    const minBandDisplay = Math.max(0.0025, baseBandDisplay * 0.35);
    const maxBandDisplay = Math.min(0.35, baseBandDisplay * (3 + bucketsForDisplay * 0.5));
    const displayBandWidth = minBandDisplay + (maxBandDisplay - minBandDisplay) * coverageEased;
    ...
    React.useEffect(() => {
        ...
        const coverageEased = Math.pow(coverage, 0.75);
        const activeCount = Math.max(1, Math.min(buckets, Math.round(1 + coverageEased * (buckets - 1))));
        const baseBand = defaults.crackBandWidth || 0.008;
        const minBand = Math.max(0.0025, baseBand * 0.35);
        const maxBand = Math.min(0.35, baseBand * (3 + buckets * 0.5));
        const computedBand = minBand + (maxBand - minBand) * coverageEased;
        const nextParams = {
            ...params,
