...
const App: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        const minBand = Math.max(0.0005, baseBand * 0.28);
        const maxBandTarget = Math.min(0.95, Math.max(bucketSpan, baseBand * 24));
        const coverageForBand = Math.pow(Math.min(1, Math.max(0, coverage)), 0.7);
        const computedBand = minBand + (maxBandTarget - minBand) * coverageForBand;
        const nextParams = {
            ...params,
