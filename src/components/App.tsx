...
const App: React.FC = () => {
    ...
    const clampCrackDivisions = (value: number) => {
        if (!isFinite(value)) return 600;
        const rounded = Math.round(value);
        return Math.max(100, Math.min(3000, rounded));
    };
    const clampCrackThickness = (value: number) => {
        if (!isFinite(value)) return 6;
        return Math.max(1, Math.min(20, value));
    };
    const clampCrackDilate = (value: number) => {
        if (!isFinite(value)) return 2;
        return Math.max(0, Math.min(12, value));
    };
    const clampCrackQualityValue = (value: number) => {
        if (!isFinite(value)) return 1;
        return Math.max(0.3, Math.min(3.0, value));
    };
    const clampCrackPatternScale = (value: number) => {
        if (!isFinite(value)) return 1;
        return Math.max(0.4, Math.min(2.5, value));
    };
    const crackDefaultsRef = React.useRef<{
        divisions: number;
        thickness: number;
        dilateRadius: number;
        quality: number;
    ...
    if (!crackDefaultsRef.current) {
        const params = ((config as any).render?.crackProceduralParams) || {};
        const defaults = {
            divisions: clampCrackDivisions(params.divisions ?? 600),
            thickness: clampCrackThickness(params.thickness ?? 6),
            dilateRadius: clampCrackDilate(params.dilateRadius ?? 2),
            quality: clampCrackQualityValue(params.quality ?? 1),
            patternScale: clampCrackPatternScale(params.patternScale ?? 1),
        };
        ...
        crackDefaultsRef.current = defaults;
    }
    const crackDefaults = crackDefaultsRef.current!;
    const [crackDivisions, setCrackDivisions] = useState<number>(() => clampCrackDivisions(safeLoadNumber('crack.divisions', crackDefaults.divisions)));
    const [crackThicknessValue, setCrackThicknessValue] = useState<number>(() => clampCrackThickness(safeLoadNumber('crack.thickness', crackDefaults.thickness)));
    const [crackDilateValue, setCrackDilateValue] = useState<number>(() => clampCrackDilate(safeLoadNumber('crack.dilateRadius', crackDefaults.dilateRadius)));
    const [crackQualityValue, setCrackQualityValue] = useState<number>(() => clampCrackQualityValue(safeLoadNumber('crack.quality', crackDefaults.quality)));
    const [crackPatternScale, setCrackPatternScale] = useState<number>(() => clampCrackPatternScale(safeLoadNumber('crack.patternScale', crackDefaults.patternScale)));
    useEffect(() => {
        const sanitizedDivisions = clampCrackDivisions(crackDivisions);
        const sanitizedThickness = clampCrackThickness(crackThicknessValue);
        const sanitizedDilate = clampCrackDilate(crackDilateValue);
        const sanitizedQuality = clampCrackQualityValue(crackQualityValue);
        const sanitizedPattern = clampCrackPatternScale(crackPatternScale);
        const current = { ...(((config as any).render?.crackProceduralParams) || {}) };
        (config as any).render = {
            ...(config as any).render,
            crackProceduralParams: {
                ...current,
                ...
                patternScale: sanitizedPattern,
            },
        };
        try { localStorage.setItem('crack.divisions', String(sanitizedDivisions)); } catch (e) {}
        try { localStorage.setItem('crack.thickness', String(sanitizedThickness)); } catch (e) {}
        try { localStorage.setItem('crack.dilateRadius', String(sanitizedDilate)); } catch (e) {}
        try { localStorage.setItem('crack.quality', String(sanitizedQuality)); } catch (e) {}
        try { localStorage.setItem('crack.patternScale', String(sanitizedPattern)); } catch (e) {}
    }, [crackDivisions, crackThicknessValue, crackDilateValue, crackQualityValue, crackPatternScale]);
    ...
    return (
        <div id="main-viewport-container">
            ...
            <div id="control-bar" className={controlsCollapsed ? 'collapsed' : ''}>
                ...
                {/* Procedural crack configuration */}
                <div style={{ display: 'inline-block', marginLeft: 12, padding: '6px', border: '1px solid #444', borderRadius: 6 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6 }}>Configuração das rachaduras</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 320 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 12, width: 170 }}>Quantidade de células</span>
                            <input
                                type="range"
                                min={100}
                                max={3000}
                                step={1}
                                value={crackDivisions}
                                onChange={(e) => setCrackDivisions(clampCrackDivisions(parseInt(e.target.value, 10)))}
                                style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: 12, width: 56, textAlign: 'right' }}>{Math.round(crackDivisions)}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 12, width: 170 }}>Espessura base</span>
                            <input
                                type="range"
                                min={1}
                                max={20}
                                step={0.1}
                                value={crackThicknessValue}
                                onChange={(e) => setCrackThicknessValue(clampCrackThickness(parseFloat(e.target.value)))}
                                style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: 12, width: 56, textAlign: 'right' }}>{crackThicknessValue.toFixed(1)}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 12, width: 170 }}>Dilatação (raio)</span>
                            <input
                                type="range"
                                min={0}
                                max={12}
                                step={0.1}
                                value={crackDilateValue}
                                onChange={(e) => setCrackDilateValue(clampCrackDilate(parseFloat(e.target.value)))}
                                style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: 12, width: 56, textAlign: 'right' }}>{crackDilateValue.toFixed(1)}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 12, width: 170 }}>Escala do padrão</span>
                            <input
                                type="range"
                                min={0.4}
                                max={2.5}
                                step={0.05}
                                value={crackPatternScale}
                                onChange={(e) => setCrackPatternScale(clampCrackPatternScale(parseFloat(e.target.value)))}
                                style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: 12, width: 56, textAlign: 'right' }}>{crackPatternScale.toFixed(2)}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 12, width: 170 }}>Qualidade (resolução)</span>
                            <input
                                type="range"
                                min={0.3}
                                max={3}
                                step={0.05}
                                value={crackQualityValue}
                                onChange={(e) => setCrackQualityValue(clampCrackQualityValue(parseFloat(e.target.value)))}
                                style={{ flex: 1 }}
                            />
                            <span style={{ fontSize: 12, width: 56, textAlign: 'right' }}>{crackQualityValue.toFixed(2)}</span>
                        </div>
                        <div style={{ fontSize: 11, opacity: 0.75 }}>Ajuste os valores antes de clicar em “Gerar mapa”.</div>
                    </div>
                </div>
