...
const App: React.FC = () => {
    ...
    const easedCoverageForDisplay = Math.pow(Math.min(1, Math.max(0, crackAreaCoverage)), 0.75);
    const activeBucketsForDisplay = Math.max(1, Math.min(bucketsForDisplay, Math.round(1 + easedCoverageForDisplay * (bucketsForDisplay - 1))));
    ...
    const minBandDisplay = Math.max(0.001, baseBandDisplay * 0.45);
    const maxBandDisplay = Math.min(0.18, baseBandDisplay * 7.5);
    const displayBandWidth = minBandDisplay + (maxBandDisplay - minBandDisplay) * easedCoverageForDisplay;
    ...
    React.useEffect(() => {
        ...
        const easedCoverage = Math.pow(coverage, 0.75);
        const activeCount = Math.max(1, Math.min(buckets, Math.round(1 + easedCoverage * (buckets - 1))));
        const baseBand = defaults.crackBandWidth || 0.008;
        const minBand = Math.max(0.001, baseBand * 0.45);
        const maxBand = Math.min(0.18, baseBand * 7.5);
        const computedBand = minBand + (maxBand - minBand) * easedCoverage;
        const nextParams = {
            ...params,
