...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, roadCrackTexture, roadCrackScale, roadCrackAlpha, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    const buildProceduralCrackGraphics = (spriteW: number, spriteH: number, minX: number, minY: number) => {
        ...
        if (canvasW > maxCanvasDimension || canvasH > maxCanvasDimension || canvasW * canvasH > maxCanvasPixels) {
            console.warn('[GameCanvas] Procedural crack overlay skipped – area too large after clamping', { canvasW, canvasH, quality });
            return null;
        }
        ...
        const invQuality = 1 / quality;
        const crackColor = (typeof renderCfg.crackProceduralColor === 'number') ? renderCfg.crackProceduralColor : 0x181818;
        const graphics = new PIXI.Graphics();
        let hasGeometry = false;
        for (let y = 0; y < canvasH; y++) {
            let runStart = -1;
            for (let x = 0; x <= canvasW; x++) {
                const alpha = (x < canvasW) ? data[(y * canvasW + x) * 4 + 3] : 0;
                if (alpha > 0) {
                    if (runStart === -1) runStart = x;
                } else if (runStart !== -1) {
                    const runEnd = x;
                    const width = (runEnd - runStart) * invQuality;
                    const height = invQuality;
                    if (width > 0 && height > 0) {
                        if (!hasGeometry) {
                            graphics.beginFill(crackColor, 1.0);
                            hasGeometry = true;
                        }
                        graphics.drawRect(runStart * invQuality, y * invQuality, width, height);
                    }
                    runStart = -1;
                }
            }
    ...
    // Try to update the existing crack overlay in-place when props change.
    // If there's no display object currently attached, trigger a lightweight rebuild of overlays.
    React.useEffect(() => {
        try {
            const display = roadCrackDisplayRef.current;
            const alphaVal = (typeof roadCrackAlpha === 'number') ? roadCrackAlpha : undefined;
            if (display) {
                if ((display as any) instanceof PIXI.TilingSprite && (display as any).tileScale) {
                    const scaleVal = (typeof roadCrackScale === 'number' && isFinite(roadCrackScale)) ? Math.max(1e-6, roadCrackScale) : 1.0;
                    (display as any).tileScale.set(scaleVal, scaleVal);
                    if (typeof alphaVal === 'number') (display as any).alpha = alphaVal;
                } else if ((display as any) instanceof PIXI.Sprite) {
                    if (typeof alphaVal === 'number') (display as any).alpha = alphaVal;
                } else if ((display as any) instanceof PIXI.Graphics) {
                    if (typeof alphaVal === 'number') (display as any).alpha = alphaVal;
                }
            } else {
                try { onMapChange(false); } catch (e) {}
    ...
    // Removido overlay de zonas em tiles (agora usamos canvas overlay Perlin)
        ...
        try {
            try { roadCrackDisplayRef.current = null; } catch (e) {}
            ...
            if (shouldRenderCracks) {
                ...
                if (isFinite(minX) && isFinite(minY) && maxX > minX && maxY > minY) {
                    ...
                    let displayToUse: PIXI.DisplayObject | null = null;
                    if (useProceduralCracks) {
                        const procedural = buildProceduralCrackGraphics(spriteW, spriteH, minX, minY);
                        if (procedural && procedural.graphics) {
                            const prev = proceduralCrackGraphicsRef.current;
                            if (prev && prev !== procedural.graphics) {
                                try { prev.destroy(); } catch (e) {}
                            }
                            proceduralCrackGraphicsRef.current = procedural.graphics;
                            displayToUse = procedural.graphics;
                        } else {
                            if (proceduralCrackGraphicsRef.current) {
                                try { proceduralCrackGraphicsRef.current.destroy(); } catch (e) {}
                            }
                    ...
                    if (!displayToUse && allowTexture && textureFromProps) {
                            ...
                            if (bt) {
                                const applyWrap = () => {
                                    ...
                                    try {
                                        ...
                                        if (roadCrackDisplayRef.current && (roadCrackDisplayRef.current as any) instanceof PIXI.TilingSprite && (roadCrackDisplayRef.current as any).tileScale) (roadCrackDisplayRef.current as any).tileScale.set(sv, sv);
                                    } catch (e) {}
                                };
                    ...
                    if (displayToUse) {
                        if ((displayToUse as any) instanceof PIXI.Graphics || (displayToUse as any) instanceof PIXI.Sprite || (displayToUse as any) instanceof PIXI.TilingSprite) {
                            (displayToUse as any).alpha = (typeof roadCrackAlpha === 'number') ? roadCrackAlpha : 0.6;
                        }
                        displayToUse.x = 0;
                        displayToUse.y = 0;
