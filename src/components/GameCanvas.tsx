...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, roadCrackTexture, roadCrackScale, roadCrackAlpha, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    const buildProceduralCrackGraphic = (spriteW: number, spriteH: number, minX: number, minY: number) => {
        ...
        if (canvasW > maxCanvasDimension || canvasH > maxCanvasDimension || canvasW * canvasH > maxCanvasPixels) {
            console.warn('[GameCanvas] Procedural crack draw skipped – area too large after clamping', { canvasW, canvasH, quality });
            return null;
        }
        ...
        const baseDilate = (typeof crackCfg.dilateRadius === 'number' && crackCfg.dilateRadius >= 0) ? crackCfg.dilateRadius : 2;
        const thicknessScale = (typeof roadCrackScale === 'number' && isFinite(roadCrackScale)) ? Math.max(0.25, roadCrackScale) : 1;
        const thickness = baseThickness * thicknessScale;
        const dilateRadius = baseDilate * thicknessScale;
        ...
        const pxSizeX = spriteW / canvasW;
        const pxSizeY = spriteH / canvasH;
        ...
        for (let y = 0; y < canvasH; y++) {
            ...
            for (let x = 0; x < canvasW; x++) {
                ...
                if (alpha > 0) {
                    ...
                    const runEnd = x;
                    const avgAlpha = count > 0 ? alphaSum / count : 0;
                    const normAlpha = Math.max(0, Math.min(1, avgAlpha / 255));
                    if (normAlpha > 0) {
                        graphics.beginFill(color, normAlpha);
                        graphics.drawRect(runStart * pxSizeX, y * pxSizeY, Math.max(pxSizeX, (runEnd - runStart) * pxSizeX), pxSizeY);
                        graphics.endFill();
                        hasGeometry = true;
                    }
                    runStart = -1;
        ...
        try {
            ...
            if (shouldRenderCracks) {
                ...
                if (isFinite(minX) && isFinite(minY) && maxX > minX && maxY > minY) {
                    ...
                    let crackDisplay: PIXI.DisplayObject | null = null;
                    if (useProceduralCracks) {
                        const proceduralGraphic = buildProceduralCrackGraphic(spriteW, spriteH, minX, minY);
                        if (proceduralGraphic) {
                            proceduralGraphic.x = 0;
                            proceduralGraphic.y = 0;
                            proceduralGraphic.alpha = (typeof roadCrackAlpha === 'number') ? roadCrackAlpha : 0.65;
                            crackDisplay = proceduralGraphic;
                        }
                    }

                    if (!crackDisplay && allowTexture && textureFromProps) {
                        const sprite = new PIXI.Sprite(textureFromProps);
                    ...
                    if (crackDisplay) {
                        ...
                        container.x = minX;
                        container.y = minY;
                        container.addChild(crackDisplay);
