...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, roadCrackTexture, roadCrackScale, roadCrackAlpha, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    }, [edgeTexture, roadCrackScale, roadCrackAlpha, roadCrackTexture]);
    useEffect(() => {
    ...
    const buildProceduralCrackField = (spriteW: number, spriteH: number, minX: number, minY: number) => {
        ...
        const scaleOverride = (typeof renderCfg.roadCrackTextureScale === 'number' && isFinite(renderCfg.roadCrackTextureScale)) ? Math.max(0.25, Math.min(4, renderCfg.roadCrackTextureScale)) : 1;
        ...
        let quality = clampQuality((fallbackQuality || 1) * scaleOverride);
        ...
        if (canvasW > maxCanvasDimension || canvasH > maxCanvasDimension || canvasW * canvasH > maxCanvasPixels) {
            console.warn('[GameCanvas] Procedural crack field skipped – area too large after clamping', { canvasW, canvasH, quality });
            return null;
        }
        ...
        if ((config as any).render?.crackUseNoise) {
            ...
            const pixelW = spriteW / canvasW;
            const pixelH = spriteH / canvasH;
            ...
            for (let y = 0; y < canvasH; y++) {
                for (let x = 0; x < canvasW; x++) {
                    ...
                    if (alpha === 0) continue;
                    const screenX = (x + 0.5) * pixelW;
                    const screenY = (y + 0.5) * pixelH;
        ...
        return { pixels: data, width: canvasW, height: canvasH, quality, color: 0x181818 };
    };
    ...
    // Removido overlay de zonas em tiles (agora usamos canvas overlay Perlin)
        ...
        try {
            ...
            const shouldRenderCracks = useProceduralCracks;
            try { console.debug('[GameCanvas] crack overlay -> procedural=', useProceduralCracks); } catch (e) {}
            if (shouldRenderCracks) {
                ...
                if (isFinite(minX) && isFinite(minY) && maxX > minX && maxY > minY) {
                    ...
                    if (spriteW > 0 && spriteH > 0 && useProceduralCracks) {
                        const field = buildProceduralCrackField(spriteW, spriteH, minX, minY);
                        if (field) {
                            const { pixels, width: fieldW, height: fieldH, color } = field;
                            if (fieldW > 0 && fieldH > 0) {
                                const pixelW = spriteW / fieldW;
                                const pixelH = spriteH / fieldH;
                                const cracksG = new PIXI.Graphics();
                                const fillColor = typeof color === 'number' ? color : 0x181818;
                                const fillAlpha = (() => {
                                    const base = (typeof roadCrackAlpha === 'number') ? roadCrackAlpha : 0.6;
                                    if (!isFinite(base)) return 0.6;
                                    return Math.max(0, Math.min(1, base));
                                })();
                                let hasDrawn = false;
                                cracksG.beginFill(fillColor, fillAlpha);
                                for (let y = 0; y < fieldH; y++) {
                                    let runStart = -1;
                                    for (let x = 0; x < fieldW; x++) {
                                        const idx = (y * fieldW + x) * 4;
                                        const hasCrack = pixels[idx + 3] > 0;
                                        if (hasCrack) {
                                            if (runStart === -1) runStart = x;
                                        } else if (runStart !== -1) {
                                            const runLength = x - runStart;
                                            if (runLength > 0) {
                                                cracksG.drawRect(runStart * pixelW, y * pixelH, runLength * pixelW, pixelH);
                                                hasDrawn = true;
                                            }
                                            runStart = -1;
                                        }
                                    ...
                                    if (runStart !== -1) {
                                        const runLength = fieldW - runStart;
                                        if (runLength > 0) {
                                            cracksG.drawRect(runStart * pixelW, y * pixelH, runLength * pixelW, pixelH);
                                            hasDrawn = true;
                                        }
                                ...
                                if (hasDrawn) {
                                    ...
                                    try {
                                        console.debug('[GameCanvas] roadCrackOverlay added container, childrenNow=', roadCrackOverlay.current?.children.length);
                                    } catch (e) {}
                                }
                            }
                        }
