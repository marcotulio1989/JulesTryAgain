...
const GameCanvas: React.FC<GameCanvasPropsInternal> = ({ interiorTexture, interiorTextureScale, interiorTextureAlpha, interiorTextureTint, crossfadeEnabled, crossfadeMs, roadCrackTexture, roadCrackScale, roadCrackAlpha, edgeTexture, edgeScale, edgeAlpha, roadLaneTexture, roadLaneScale, roadLaneAlpha }) => {
    ...
    const noiseViewRef = useRef<{ cameraX: number; cameraY: number; zoom: number }>({ cameraX: 0, cameraY: 0, zoom: 1 });
    const noiseViewLastUpdateRef = useRef<number>(0);
                    ...
                    if (useProceduralCracks) {
                        ...
                        if (renderCfg.showNoiseDelimitations && raster.debugRegion) {
                            try {
                                ...
                                const activeFillAlpha = hasHighlight ? 0.32 : 0.22;
                                const inactiveFillAlpha = hasHighlight ? 0.04 : 0.12;
                                const activeStrokeAlpha = hasHighlight ? 0.85 : 0.55;
                                const inactiveStrokeAlpha = hasHighlight ? 0.35 : 0.3;
                                const g = new PIXI.Graphics();
                                for (let ry = 0; ry < region.h; ry++) {
                                    for (let rx = 0; rx < region.w; rx++) {
                                        ...
                                        const strokeAlpha = isActive ? activeStrokeAlpha : inactiveStrokeAlpha;
                                        const fillAlpha = isActive ? activeFillAlpha : inactiveFillAlpha;
                ...
                const animate = () => {
                    if (state.initialised && stage.current && pixiRenderer.current) {
                        ...
                        try {
                            ...
                            // Atualizar view do overlay de ruído para acompanhar pan/zoom
                            try {
                                const nextView = { cameraX: state.camera.x, cameraY: state.camera.y, zoom: state.zoom };
                                const prevView = noiseViewRef.current;
                                const deltaCam = Math.abs(prevView.cameraX - nextView.cameraX)
                                    + Math.abs(prevView.cameraY - nextView.cameraY);
                                const deltaZoom = Math.abs(prevView.zoom - nextView.zoom);
                                if (deltaCam > 1e-4 || deltaZoom > 1e-4) {
                                    if (typeof NoiseZoning.setView === 'function') {
                                        NoiseZoning.setView(nextView);
                                    }
                                    noiseViewRef.current = nextView;
                                }
                                if (NoiseZoning.enabled && typeof NoiseZoning.redraw === 'function') {
                                    const nowTick = Date.now();
                                    const shouldForce = deltaCam > 0.5 || deltaZoom > 1e-3;
                                    const timeElapsed = nowTick - noiseViewLastUpdateRef.current;
                                    if (shouldForce || ((deltaCam > 1e-4 || deltaZoom > 1e-4) && timeElapsed > 120)) {
                                        NoiseZoning.redraw();
                                        noiseViewLastUpdateRef.current = nowTick;
                                    }
                                }
                            } catch (e) {
                                // manter overlay opcional seguro
                            }
                        } catch (e) {
                            // non-fatal
                        }
