                    ...
                    if (useProceduralCracks) {
                        ...
                        if (renderCfg.showNoiseDelimitations && raster.debugRegion) {
                            try {
                                ...
                                const outlineAlphaInactive = 0.18;
                                const outlineAlphaActive = 0.82;
                                const baseFillAlpha = 0.16;
                                const activeFillAlpha = 0.34;
                                const tintColor = 0x38bdf8;
                                const g = new PIXI.Graphics();
                                for (let ry = 0; ry < region.h; ry++) {
                                    for (let rx = 0; rx < region.w; rx++) {
                                        ...
                                        const outlineAlpha = isActive ? outlineAlphaActive : outlineAlphaInactive;
                                        g.lineStyle(strokeWidth, tintColor, outlineAlpha);
                                        const drawRect = () => g.drawRect(rx * safeCellW, ry * safeCellH, safeCellW, safeCellH);
                                        if (isActive) {
                                            g.beginFill(tintColor, activeFillAlpha);
                                            drawRect();
                                            g.endFill();
                                        } else if (!hasHighlight) {
                                            g.beginFill(tintColor, baseFillAlpha);
                                            drawRect();
                                            g.endFill();
                                        } else {
                                            drawRect();
