                    ...
                    if (useProceduralCracks) {
                        ...
                        if (renderCfg.showNoiseDelimitations && raster.debugRegion) {
                            try {
                                ...
                                const gridStrokeColor = 0x334155;
                                const activeColor = 0x38bdf8;
                                const baseFillAlpha = hasHighlight ? 0 : 0.18;
                                const activeFillAlpha = hasHighlight ? 0.35 : 0.24;
                                const g = new PIXI.Graphics();
                                for (let ry = 0; ry < region.h; ry++) {
                                    for (let rx = 0; rx < region.w; rx++) {
                                        ...
                                        const strokeColor = isActive ? activeColor : gridStrokeColor;
                                        const strokeAlpha = isActive ? 0.9 : 0.25;
                                        const fillAlpha = isActive ? activeFillAlpha : baseFillAlpha;
                                        g.lineStyle(strokeWidth, strokeColor, strokeAlpha);
                                        if (fillAlpha > 0) {
                                            g.beginFill(isActive ? activeColor : gridStrokeColor, fillAlpha);
