...
const CracksPreview: React.FC<CracksPreviewProps> = ({ width = DEFAULT_WIDTH, height = DEFAULT_HEIGHT, onApplyCanvas }) => {
    ...
    const voronoiDefaults = ((config as any).render?.crackVoronoiParams) || {};
    const [divisions, setDivisions] = useState<number>(() => typeof voronoiDefaults.divisions === 'number' ? voronoiDefaults.divisions : 400);
    const [thickness, setThickness] = useState<number>(() => typeof voronoiDefaults.thickness === 'number' ? voronoiDefaults.thickness : 6); // corresponds to eps slider in original (div by 10 later)
    const [seed, setSeed] = useState<number>(() => Date.now());
    const [quality, setQuality] = useState<number>(1); // 1,2,4
    const [applyTarget, setApplyTarget] = useState<ApplyTarget>('road');
    const [dilateRadius, setDilateRadius] = useState<number>(() => typeof voronoiDefaults.dilateRadius === 'number' ? voronoiDefaults.dilateRadius : 2); // pixels to expand edges (helps cover curves)
    ...
    function generateCanvas(scale = 1): HTMLCanvasElement {
        const canvas = generateVoronoiCrackCanvas({
            width,
            height,
            scale,
            divisions,
