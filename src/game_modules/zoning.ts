...
import { warpedSimplexNoise, warpedSimplexNoiseSigned } from '../tools/noiseSampler';
...
const Zoning = new (class {
  ...
  private _macroNoise(x: number, y: number): number {
    ...
    return warpedSimplexNoiseSigned(this._noise, x * mn.baseScale, y * mn.baseScale, mn.octaves, mn.lacunarity, mn.gain); // [-1,1]
  }
  ...
  if (config.zoningModel.mode === 'concentric') {
      ...
      const n = warpedSimplexNoise(this._noise, p.x * baseScale, p.y * baseScale, octaves, lacunarity, gain);
